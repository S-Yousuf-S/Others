...
def find_relevant_sources(question, documents):
    ...
    for source, text in documents.items():
        ...
        for block in text.split("\n\n"):
            ...
            # Split huge blocks into smaller semantic chunks
            if len(block) > 800:
        ...
        for chunk in chunks:
            ...
            # Penalize extremely large noisy chunks
            if len(chunk) > 1500:
                continue
            ...
            if score >= threshold:
                ...
                # Smaller context window for precision
                context_window = chunks[
                    max(0, chunk_index):
                    min(len(chunks), chunk_index + 1)
                ]

                expanded_chunk = "\n\n".join(context_window)

                scored_chunks.append(
                    (score, expanded_chunk.strip())
                )
...
if question and documents and st.session_state.api_configured:
    ...
    if "just_rerendered" not in st.session_state:
        st.session_state.just_rerendered = True
        st.rerun()

    st.session_state.just_rerendered = False    
    st.markdown("""
    <div class="footer">
